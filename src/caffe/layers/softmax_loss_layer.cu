...
namespace caffe {
	...
	void SoftmaxWithLossLayer<Dtype>::Forward_gpu(
		...
		// on the backward pass, we use it here to avoid having to allocate
		// new GPU
		// memory to accumulate intermediate results in the kernel.
		Dtype *loss_data = bottom[0]->mutable_gpu_diff();
		// Similarly, this memory is never used elsewhere, and thus we can use
		// it
		...
		SoftmaxLossForwardGPU<Dtype>
			<<<CAFFE_GET_BLOCKS(nthreads), CAFFE_CUDA_NUM_THREADS>>>(
				nthreads, prob_data, label, weight, loss_data, outer_num_,
				dim, inner_num_, has_ignore_label_, ignore_label_, counts);
		Dtype loss = Dtype(0.0);
		caffe_gpu_asum(nthreads, loss_data, &loss);
		Dtype valid_count = -1;
		// Only launch another CUDA kernel if we actually need the count of
		// valid
		// outputs.
	...
	void SoftmaxWithLossLayer<Dtype>::Backward_gpu(
		...
		caffe_gpu_memcpy(prob_.count() * sizeof(Dtype), prob_data,
							bottom_diff);
		const Dtype *label = bottom[1]->gpu_data();
		bool has_weight = bottom.size() >= 3;
		const Dtype *weight = NULL;
		...
		SoftmaxLossBackwardGPU<Dtype>
			<<<CAFFE_GET_BLOCKS(nthreads), CAFFE_CUDA_NUM_THREADS>>>(
				nthreads, top_data, label, weight, bottom_diff, outer_num_, dim,
